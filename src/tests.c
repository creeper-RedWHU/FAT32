...
// Tests
int test_basic_read(char *input_fle, char *expected);
int test_basic_write(char *input_file, char *write_string);
int test_basic_append(char *input_file, char *write_string);

int main(int argc, char **argv) {

    const char *img = (argc > 1) ? argv[1] : "scripts/test.fat32";

    printf("\nFAT32 Filesystem Test\n");

    /* 先附着镜像文件 */
    if (tf_attach_image(img) != 0) {
        fprintf(stderr, "[ERR] tf_attach_image failed for %s\n", img);
        return 1;
    }
    /* 再初始化文件系统结构 */
    int rc = tf_init();
    printf("[DEBUG] tf_init rc=%d\n",rc);
    if (rc != 0) {
        fprintf(stderr, "[ERR] tf_init failed\n");
        return 1;
    }
